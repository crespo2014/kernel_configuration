...
)
{
    ...
    if (rmStatus != NV_OK)
    {
        nv_printf(NV_DBG_INFO,
                  "NVRM: Unable to read UUID for GPU " NV_PCI_DEV_FMT ": 0x%x\n",
                  NV_PCI_DEV_FMT_ARGS(nv), rmStatus);
        return;
    }
    ...
    if (nv_is_uuid_in_gpu_blacklist(uuid))
    {
        nv_printf(NV_DBG_WARNINGS,
                  "NVRM: Could not blacklist %s because PBI is not supported\n",
                  uuid);
        WARN_ON(1);
    }
...
)
{
    ...
    if (rm_status != NV_OK)
    {
        nv_printf(NV_DBG_INFO,
                  "NVRM: Unable to read UUID for GPU " NV_PCI_DEV_FMT ": 0x%x\n",
                  NV_PCI_DEV_FMT_ARGS(nv), rm_status);
        return;
    }
    ...
    if (nv_is_uuid_in_gpu_blacklist(uuid_str))
    {
        ...
        if (rm_status != NV_OK)
        {
            nv_printf(NV_DBG_ERRORS, "NVRM: Failed to blacklist %s: 0x%x\n",
                      uuid_str, rm_status);
            goto done;
        }
        nv->flags |= NV_FLAG_BLACKLIST;
        nv_printf(NV_DBG_INFO, "NVRM: Blacklisted %s successfully\n",
                  uuid_str);
    }
...
static int nv_start_device(nv_state_t *nv, nvidia_stack_t *sp)
{
    ...
    if (!NV_IS_GVI_DEVICE(nv))
    {
        if (!(nv->flags & NV_FLAG_PERSISTENT_SW_STATE))
        {
            ...
            if (msi_config == 1)
            {
                if (pci_find_capability(nvl->dev, PCI_CAP_ID_PM))
                {
                    if (pci_find_capability(nvl->dev, PCI_CAP_ID_MSIX))
                    {
                        nv_init_msix(nv);
                    }
                    if (pci_find_capability(nvl->dev, PCI_CAP_ID_MSI) &&
                ...
                else
                {
                    ...
                    nv_printf(NV_DBG_INFO,
                              ...
                              "NVRM: Not enabling MSIX or MSI interrupts.\n");
                }
            }
        }
    }
    ...
    else
    {
        ...
        if (!(nv->flags & NV_FLAG_PERSISTENT_SW_STATE))
        {
            ...
            {
                rc = request_irq(nv->interrupt_line, nvidia_isr, nv_default_irq_flags(nv),
                                 nv_device_name, (void *)nvl);
            }
        }
    }
...
static int nv_open_device(nv_state_t *nv, nvidia_stack_t *sp)
{
    ...
    else if (rm_is_device_sequestered(sp, nv))
    {
        ...
        nv_printf(NV_DBG_ERRORS,
                  "NVRM: device " NV_PCI_DEV_FMT " is currently unavailable\n",
...
)
{
    ...
    if ((nv->flags & NV_FLAG_BLACKLIST) != 0)
    {
        ...
        nv_printf(NV_DBG_ERRORS,
                  "NVRM: open() not permitted for blacklisted %s\n",
                                ...
                                nv_linux_state_t *nvl)
{
    ...
    /*
     * Issue the post-offline cache flush/invalidation of NUMA region.
     * This is necessary because the NVIDIA driver cannot initiate the full
     * 'physical removal' step of memory hot-unplug, during which the kernel
     * arch layer would handle this.
     */
    nv_ibmnpu_cache_flush_numa_region(nv);
    ...
    switch (arg_cmd)
    {
        ...
        case NV_ESC_SET_NUMA_STATUS:
        {
            ...
            if (nv_get_numa_status(nvl) != api->status)
            {
                ...
                {
                    /*
                     * Post-online cache flush/invalidation of NUMA region.
                     * Recommended by IBM to attempt to mitigate a condition
                     * where NUMA memory gets pulled into the cache after
                     * the post-offline flush/invalidation.
                     */
                    nv_ibmnpu_cache_flush_numa_region(nv);
...
)
{
    ...
    if ((dev->irq == 0 && !pci_find_capability(dev, PCI_CAP_ID_MSIX))
...
)
{
    ...
    return nvidia_suspend(pdev, state);
}
...
{
    /*todo : handle hibernate image creation failure */
